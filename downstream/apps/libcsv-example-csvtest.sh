...
probe() {
  ...
  downstream::run_logged "$DOWNSTREAM_LOG_DIR/csvtest.out" \
    bash -lc "'$app_root/usr/bin/$BINARY' < '$DOWNSTREAM_LOG_DIR/fixture.csv'"

  python3 - "$DOWNSTREAM_LOG_DIR/csvtest.out" <<'PY'
